...
async function handle_output(
  ...
  if (code === 0) {
    ...
    console.log(
      `✅ Repository '${repo}' successfully cloned to ${REPO_DIRECTORY}/${repo}`
    );
  } else {
...
async function create_if_not_exists(repo: string) {
  ...
  try {
    ...
    const clone = await clone_repo(
      'git@git.hubteam.com:HubSpotProtected',
      repo
    );
