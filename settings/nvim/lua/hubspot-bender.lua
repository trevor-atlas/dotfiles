...
local function is_none(value) return value == nil or value == '' end
...
local function job_logger(data)
  if is_none(data) then return end
...
local function start_asset_bender(rootsArray)
  ...
  local job = vim.system(args, {
    stdout = function(err, data) job_logger(data) end,
    stderr = function(err, data) job_logger(data) end,
    text = true,
    detach = true,
  }, function(result)
...
function M.check_start_javascript_lsp()
  ...
  if not should_handle_file(bufnr) then return end
  ...
  if contains(current_project_roots, root_dir) then return end
...
function M.stop() shutdown_current_process() end
...
function M.getTsServerPathForCurrentFile()
  local bufnr = vim.api.nvim_get_current_buf()
  if not should_handle_file(bufnr) then return LATEST end
  ...
  log_info('node_modules found at ' .. node_module_dir .. ' - will parse the package.json in that directory for the hs-typescript version')

  local packagejson_path = path_join(node_module_dir, 'package.json')
  local version_check = vim.system({ 'jq', '-r', '.bpm.deps.["hs-typescript"]', packagejson_path }, { text = true }):wait()
  ...
  local resolved_version_path = vim.system({ 'bpx', '--path', string.format('hs-typescript@%s', version) }, { text = true }):wait()

  if resolved_version_path.stderr ~= '' then
    log_error('there was an error determining the path of hs-typescript from version number: ' .. version .. '\n' .. resolved_version_path.stderr)
    return LATEST
  end
