...
(async function main() {
  try {
    const res = await execSync(
      `gh issue list --label 'FE' --repo HubSpot/Billing-Management-Team --json assignees,author,body,closed,closedAt,comments,createdAt,id,labels,milestone,number,state,title,updatedAt,url`
    );
